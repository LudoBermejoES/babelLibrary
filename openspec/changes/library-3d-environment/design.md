...
- **Verticality**: galleries are now addressed `(q, r, floor)`. Floor 0 fills via the original 2D blob-growth algorithm to a target size (7–19 hexagons), then floor 1 starts directly above the same footprint, and so on, until enough capacity exists for the catalog (`ceil(book_count / BOOKS_PER_HEX)` hexagons). A hexagon's vestibule staircase connects it to its floor+1/floor-1 counterpart at the same `(q, r)`.
